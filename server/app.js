...
// Logout endpoint (before auth middleware so it can be accessed without authentication)
app.get('/logout', (req, res) => {
	res.clearCookie('auth_token', { path: '/', sameSite: 'lax' });
	res.clearCookie('nc_session', { path: '/', sameSite: 'lax' });
...
app.use((req, res, next) => {
	if (!req.account && req.cookies && req.cookies.nc_session) {
		try {
...
const defaultMetaTags = {
	...
	image: `${config.get('public_url')}/thumbnail.png`,
	type: 'website'
};
...
const renderPage = async (req, res) => {
	...
	const configuration = {
		...
		seneVerseBackendUrl: config.get('sene_verse_backend_url'),
	};
	const props = {
		version: version,
